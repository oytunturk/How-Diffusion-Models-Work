...
def run_train():
    ...
    for ep in range(n_epoch):
        ...
        if ep%4==0 or ep == int(n_epoch-1):
            ...
            net_file = os.path.join(save_dir, f"model_{ep}.pth")
            torch.save(nn_model.state_dict(), net_file)
            print('saved model at ' + net_file)
...
def run_test():
    ...
    #model_file = "model_4.pth"
    #model_file = "model_8.pth"
    model_file = "model_31.pth"
    
    # load in model weights and set to eval mode
    net_file = os.path.join(save_dir, model_file)
    nn_model.load_state_dict(torch.load(net_file, map_location=device))
    nn_model.eval()
    print("Loaded in Model from " + net_file)
